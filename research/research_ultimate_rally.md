# Ultimate Rally Research

Also called "Ultimate Quake Rally", "URally", and "UR".

## Official Homepages

1. `http://impact.frag.com/rally/ultimate.htm`
	1. https://web.archive.org/web/19980109081512/http://impact.frag.com/rally/ultimate.htm

## Release History

Reconstructed release history.

### Promotional Video (October 31  1997)

Released as `UR_promo.zip`.

**A copy of this release has not yet been acquired.**

Mentioned on BluesNews:

> Ultimate QuakeRally Demo [Oct 31, 1997, 11:00 am ET] – Post a Comment
>
> There is a new variant on Quake Rally in the works called Ultimate Quake Rally. Ridah has released an AVI demo (1.1 MB) showing off some of the features of the new version, described thusly:
>
>> Ultimate Rally contains an entirely new set of classy sports, off-road and specialty vehicles, utilizing the advanced vehicle physics engine used in Quake Rally. Each vehicle has been fine tuned to behave exactly as they do in real life, to create the ultimate driving experience. Ultimate Rally also includes a reworked control system, designed to please beginners and experienced QR fans alike.

-- https://www.bluesnews.com/s/229420/ultimate-quakerally-demo

Links:

* https://www.bluesnews.com/files/gamemovies/ur_promo.zip

Mentioned on Redwoods:

> November 9, 1997\
> Ultimate Rally\
> Ultimate Rally is an advanced version of Quake Rally brought to you by the same people that made the original, with better driving characteristics and new cars. Somehow I didn't get around to mentioning that there is a promotional AVI (1.1MB) that leaves me lusting for more. There are screenshots of the cars on the page. I played the Screamer Rally 2 demo for 3Dfx yesterday and it reminded me of Quake Rally. Hurry up Ridah, I want to play this one.

-- [http://redwood.stomped.com/1197.html](https://web.archive.org/web/20011031110431/http://redwood.stomped.com/1197.html)

Links:

* http://impact.frag.com/rally/ultimate.htm
* http://impact.frag.com/files/UR_promo.zip


### Interview (November 28 1997)

Interview about Ultimate Rally:

* [http://www.frag.com/interviews/i-2/](https://web.archive.org/web/19980117185553/http://www.frag.com/interviews/i-2/)

Mentioned on BluesNews:

> Interviews (Updated) [Dec 02, 1997, 11:00 am ET] – Post a Comment
>
> There is an Interview with Ridah & Sumaleth of Impact Development Team on Frag.Com, talking about the upcoming Ultimate Rally variant on QuakeRally. Thanks Hanif. Speaking of Ultimate Rally, check out this cool Quake Hummer (like there's such a thing as an uncool hummer).

-- https://www.bluesnews.com/s/229863/interviews-updated


### Progress Update (January 16 1998)

Mentioned on the Impact Development page:

> MORBO RELEASED!	. 	. 	Friday 16/1/98
>
> OK, that's another project finished and out the door ready for public consumption. I hope he proves to be fun. I now plan to spend this weekend finishing off my remaining work on Ultimate Rally (ooh, there's a name we haven't heard for a while) which basically consists of just some map work. There's some coding left for Ridah to do but I'm not sure when he'll be able to get to that, but UR will be released at some point! Stecki's cars will not go unused!

-- [http://impact.frag.com/](https://web.archive.org/web/19980206044911/http://impact.frag.com/)

### Cancelled (February 17 1998)

Mentioned on the Impact Development page:

> Zumlin skin WIP/The future of Eraser, UR?	. 	Tuesday, 17/2/98
>
> OK, for those of you who are still visiting this page in seek of UR (Ultimate Rally) news or info, this is for you. I have posponed this long enough, but for the record, UR will never be completed. I know a lot of you must be aiming a dangerous weapon at me right now, but I'm afraid it fell victim to the on-slaught of the Quake2 engine. The moment it was release, I found myself wanting to learn more and more about it, which ultimately led to the demise of UR, code-wise. This is a sad thing for me to say, since it was looking to be a great game, with some of the best replica vehicle models I've seen (by stecki, over at Deconstruct) and the code additions, I truly believe it was approaching commercial quality, with the exception of the maps.
Now, it is possible that I'll get a chance to fix up the few major tasks that remained, and get the package ready for some kind of premature release, but from history, things don't go that easily. A great deal of time is required to put together such a large package for such a large audience. I would estimate this time at being 3 or 4 days of continuous effort, and right now, I simply don't have that time. And when you factor in the significant decrease in demand for Quake1 engine games, I think that sadly, it just isn't going to happen.
>
> My sincere apologies to those who have been avidly following this page for UR news, but for the above reasons, UR is vaporware(tm). This marks my first project that has started and not finished, and is something I am not at all pleased about. I pride myself on getting the job done, and done to the best of my ability, *sigh*. Oh well, hopefully in the future, timing won't quite be of such importance (highly unlikely as long as I continue to work in the gaming community).

-- [http://impact.frag.com/](https://web.archive.org/web/19980224192055/http://impact.frag.com/)

Mentioned on BluesNews:

> Ultimate Rally Dead: "Mini" QuakeRally 1.3 Not? [Feb 17, 1998, 11:00 am ET] – Post a Comment
>
> Ridah posted some sad news on the Impact Development Team page, that Ultimate Rally, the planned uber-sequel to QuakeRally (the Quake driving conversion), will not be completed: a victim of the lure of programming Quake II over Quake. There is still hope for QuakeRally fans, however, as Sumaleth posted a subsequent update saying he would see what he could do to package the work that's been done on Ultimate Rally as a "mini" QuakeRally 1.3 release. Thanks rdwarlock.

-- https://www.bluesnews.com/s/230974/ultimate-rally-dead-mini-quakerally-1-3-not

Mentioned on Redwoods:

> February 17, 1998\
> Bye Bye Ultimate Rally\
> D'oh! Word on the Impact page is that work on Ultimate Rally is no more, but Sumaleth will try to get what is there together for a mini 1.3 rally release. That kind of bites, but Quake II is having it's effect on Quake 1 programming I guess. Now we just need a Rally for Quake II. :o) I saw this tidbit on Blue's.

-- [http://redwood.stomped.com/298.html](https://web.archive.org/web/20011121173546/http://redwood.stomped.com/298.html)


### Release of Ultimate Rally Cars (March 08 1998)

Cars developed for Ultimate Rally were released for Quake Rally v1.2.

Released as:

* `hummer.ZIP`
* `fwhl_oz.ZIP`

Mentioned on BluesNews:

> Quake Mods (Updated) [Mar 08, 1998, 11:00 am ET] – Post a Comment
>
> Used Quake Car Lot
> On Deconstruct is a set of all the cars created for the never completed Ultimate Rally, converted for use with QuakeRally 1.2, the final released version of this classic driving mod for classic Quake (available on the Impact Website). The archive is available in three varieties: GL, non-GL, and original (unscented?) which distorts the cars, but is probably the most useful variety for mod authors who want to use them in there own project (which is permitted).

-- https://www.bluesnews.com/s/231248/quake-mods-updated

Links:

* http://www.frag.com/deconstruct/

Deconstruct has a news page dedicated to his Ultimate Rally work here:

* [http://www.frag.com/deconstruct/UR/rally.htm](https://web.archive.org/web/19980117203357/http://www.frag.com/deconstruct/UR/rally.htm)

Deconstruct has a page with the release of the hummer here:

* [http://www.frag.com/deconstruct/UR/patches/patch.htm](https://web.archive.org/web/19980118045827/http://www.frag.com:80/deconstruct/UR/patches/patch.htm)

Contents of `hummer.ZIP`:

```text
-rw-rw-r--  0 0      0        3826 24 Nov  1997 hummer12.txt
-rw-rw-r--  0 0      0      215725 24 Nov  1997 hummer12.mdl
```

Contents of `fwhl_oz.ZIP`:

```text
-rw-rw-r--  0 0      0      164164 13 Dec  1997 fwhl_oz.MDL
-rw-rw-r--  0 0      0        1759 14 Dec  1997 oz_readme.txt
```

